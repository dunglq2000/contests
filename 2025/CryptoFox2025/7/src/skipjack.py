...
# O = int.from_bytes(bytes([0] * 8), 'big')
# K = int.from_bytes(bytes(list(range(12, 20))), 'big')
# sj0 = SkipJack()
# EO = sj0.encrypt(O, KEY)
# sj1 = SkipJack()
# EK = sj1.encrypt(K, KEY)
# sj2 = SkipJack()
# EEO = sj2.encrypt(EO, KEY)

# EEO = EEO.to_bytes(8, 'big')
# EK = EK.to_bytes(8, 'big')
# EO = EO.to_bytes(8, 'big')
# K = K.to_bytes(8, 'big')

# sj3 = SkipJack()
# E = sj3.encrypt(int.from_bytes(bytes(u^v for u, v in zip(EO, K))), KEY)

# E = E.to_bytes(8, 'big')

# ct = [u^v for u, v in zip(E, EK)]
# ct = [u^v for u, v in zip(ct, EEO)]
...
# Kprev = [0] * 32
# for _ in range(16):
#     Kprev = encrypt(Kroot, IV, Kprev)
#     print(bytes(Kprev).hex())
